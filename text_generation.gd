...
signal shape_json_generated(json_string)

var default_prompt = "Generate a red cube in the center of the scene"
...
var scene_schema: String = JSON.stringify(_scene_schema)
...
func _on_generate_button_pressed():
	LogDuck.d("Generate button pressed")
	$GenerateButton.disabled = true
	$GeneratedText.text = ""
	if ($SchemaOption.selected == 0):
		$Llama.run_generate_text($Prompt.text, "", scene_schema)
	else:
...
func _on_llama_generate_text_updated(new_text):
	...
	$GeneratedText.text = $GeneratedText.text.replace("<|eot_id|>", "") # Erase text generation errors
...
func _on_llama_generate_text_finished(text):
	text = text.replace("<|eot_id|>", "") # Erase text generation errors
	print("full generated text")
	print(text)
	if ($SchemaOption.selected == 0):
		emit_signal("shape_json_generated", text)
