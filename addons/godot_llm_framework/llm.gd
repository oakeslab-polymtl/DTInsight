...
func _initialize_api():
	...
	match config.provider:
		...
		LLMProviderAPI.Provider.LOCAL:
			api = LocalAPI.new()
			if debug: print("LLM: Local API initialized")
		LLMProviderAPI.Provider.OPENAI2:
			api = OpenAIAPI2.new()
			if debug: print("LLM: OpenAI2 API initialized")
...
func generate_response(prompt: String, params: Dictionary = {}, use_tools: bool = true) -> Dictionary:
	...
	if not api.supports_tool_use() || not tools || not use_tools:
		...
		append_message_history(api.extract_response_message(response)) # TODO Validate content exists
		return response
	...
	append_message_history(api.extract_response_message(response))
	
	var max_loop_limit = 7
	while api.has_tool_calls(response) && max_loop_limit > 0:
		...
		if not tool_calls.is_empty():
			...
			for tool_result in tool_results:
				append_message_history(api.format_tool_result(tool_result))
				request_params["messages"] = message_history
...
func remove_tool(tool_name: String):
	...
	for child in get_children():
		if child is BaseTool && child.tool_name == tool_name:
			remove_child(child)
			child.queue_free()
